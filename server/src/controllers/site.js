...
module.exports = {
  ...
  CountNumberOfPublicData: async (req, res) => {
    // 先搜尋快取中有沒有
    let queryHash = hash.sha1(req.query);
  ...
  getPublicSiteById: async (req, res) => {
    let { _id } = req.params;
    try {
      // 先搜尋快取中有無數據
      // 先找快取;
  ...
  countNumberOfMySite: async (req, res) => {
    let { _id } = req.user;
    try {
  ...
  getOtherSitesByAuthor: async (req, res) => {
  ...
  countNumberOfMyCollections: async (req, res) => {
    try {
  ...
  checkLikeOrCollection: async (req, res) => {
