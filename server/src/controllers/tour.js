...
module.exports = {
  ...
  countNumberOfPublicTour: async (req, res) => {
    try {
      // 先搜尋快取中有沒有
      let queryHash = hash.sha1(req.query);
  ...
  countNumberOfMyTour: async (req, res) => {
    let { _id } = req.user;
    try {
  ...
  getMyAppliedTour: async (req, res) => {
    let { _id } = req.user;
    let { page, numberPerPage } = req.query;
    try {
  ...
  getPrivateTourDetail: async (req, res) => {
    let { _id } = req.params;
  ...
  getPublicTourDetail: async (req, res) => {
    let { _id } = req.params;
    try {
      // 先搜尋快取中有無數據;
      let cacheData = await redisClient.get(`Tour:${_id}`);
  ...
  checkMyAppliedStatus: async (req, res) => {
  ...
  addSomeoneToTourist: async (req, res) => {
    try {
  ...
  deleteSiteOfTour: async (req, res) => {
  ...
  deleteSiteOverLimitDay: async (req, res) => {
